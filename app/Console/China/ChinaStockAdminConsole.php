...
class ChinaStockBusinessConsole extends Console {
	...
	const CHINA_STOCK_BUSINESS_URL  = 'http://emweb.securities.eastmoney.com/PC_HSF10/CompanyManagement/CompanyManagementAjax?code=';
	...
	public function fire(){
		...
		while(true){
			...
			foreach($listdata['list'] as $key=>$stock){
				...
				$count = $this->model('SecuritiesManagement')->where($where)->count();
				if($count){
					$this->info('已完成');
					continue;
				}
				...
				foreach($stockBusinessList['zygcfx'] as $key=>$businessList){
					...
					$count = $this->model('SecuritiesStockManagement')->where($where)->count();
					if($count){
						continue;
					}
				...
				if(!empty($businessListData)){
					$this->model('SecuritiesManagement')->data($businessListData)->addMulti();
				}
			}
			$start++;
		}
