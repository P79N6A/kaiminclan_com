...
class FinanceChinaProfitConsole extends Console {
	
	protected $intial =0;
	
	private $api = 'http://money.finance.sina.com.cn/corp/go.php/vFD_ProfitStatement/stockid/{_SYMBOL_}/ctrl/{_YEAR_}/displaytype/4.phtml';
		...
		for($year=$startYear;$year<$maxYear;$year++){
			...
			for($s=3;$s<7;$s++){
				...
				if(strtotime($temp) != false){
					...
					$balanceData = $this->model('StatementsProfit')->where($where)->find();
					if($balanceData){
						continue;
					}
					$yearList[] = $temp;
			...
			for($i=$offset; $i<$length;$i++){
				...
				if(empty($title) || $title == '报表日期' || strpos($title,'利润表') !== false){
					continue;
				}
				$startOffset = 0;
				...
				if(is_numeric(substr($title,$startOffset,1))){
					...
					$this->model('StatementsProfit')->data(array(
						'sn'=>$this->get_sn(),
