...
class FinanceChinaAssetsLiabilitiesConsole extends Console {
	
	protected $intial =0;
	private $api = 'http://money.finance.sina.com.cn/corp/go.php/vFD_BalanceSheet/stockid/{_SYMBOL_}/ctrl/{_YEAR_}/displaytype/4.phtml';
	...
	public function fire(){
		...
		foreach($listdata as $key=>$stokcData){
			...
			for($year=$startYear;$year<$maxYear;$year++){
				...
				list(,$resultText) = explode('<table id="BalanceSheetNewTable0" width="100%">',$content);
				...
				for($s=3;$s<7;$s++){
					...
					if(strtotime($temp) != false){
						...
						$balanceData = $this->model('StatementsBalance')->where($where)->find();
						if($balanceData){
							continue;
						}
						$yearList[] = $temp;
				...
				for($i=$offset; $i<$length;$i++){
					...
					if(empty($title) || $title == '报表日期' || strpos($title,'资产负债表') !== false){
						continue;
					}
					$startOffset = 0;
					...
					if(is_numeric(substr($title,$startOffset,1))){
						...
						$this->model('StatementsBalance')->data(array(
							'sn'=>$this->get_sn(),
