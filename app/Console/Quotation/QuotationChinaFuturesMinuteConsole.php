...
*/
class QuotationChinaFuturesMinuteConsole extends Console {
	private $debug=1;
	private $apiUrl = 'http://stock2.finance.sina.com.cn/futures/api/json.php/IndexService.getInnerFuturesMiniKLine5m';
		...
		foreach($listdata['list'] as $key=>$stock){
			...
			foreach($list as $cnt=>$data){
				...
				*/
				...
				$table = 'china_futures_minute_'.date('Y_W',$cycle);
