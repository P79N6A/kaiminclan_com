...
class ApproachController extends Controller {
	...
	public function fire(){
		...
		$where = array();
		$where['identity'] = $liability_subscriber_identity;
		$where['status'] = 0;
		$memberData = $this->model('ProjectMember')->where($where)->find();
		if(!$memberData){
			$this->info('工程师不存在，更换其他同事',4001);
		}
		
		$liability_subscriber_identity = $this->service('Account')->getTypeUid($this->argument('liability_subscriber_identity'),'employee');
