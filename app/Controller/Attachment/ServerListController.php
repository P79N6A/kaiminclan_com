 ...
 *
 */
class serverueListController extends Controller {
	...
	public function fire(){
		...
		$count = $this->model('ResourcesServer')->where($where)->count();
		
		$listdata = array();
		
		if($count){
			
			$listdata = $this->model('ResourcesServer')->where($where)->limit($start,$perpage,$count)->select();
