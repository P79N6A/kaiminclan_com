...
class BolsterMaintainBlock extends Block {
	
	public function getdata($param){

        $keyword = isset($param['kw'])?$param['kw']:0;

        $maintainId = isset($param['maintainId'])?$param['maintainId']:0;
        ...
        if($maintainId){
            $where['identity'] = $maintainId;
        }


        $listdata = $this->service('BolsterMaintain')->getMaintainList($where,$start,$perpage,$order);
