...
class BolsterLoopholeBlock extends Block {
	
	public function getdata($param){

        $keyword = isset($param['kw'])?$param['kw']:0;

        $loopholeId = isset($param['loopholeId'])?$param['loopholeId']:0;
        ...
        if($loopholeId){
            $where['identity'] = $loopholeId;
        }


        $listdata = $this->service('BolsterLoophole')->getLoopholeList($where,$start,$perpage,$order);
