 ...
 *
 */
class ResourcesAttachmentService extends Service {
	 ...
	 * @reutrn array;
	 */
	public function getAttachmentList($where = array(),$start = 0,$perpage = 0,$orderby = 'identity desc'){
		
		$list = array();
		
		$count = $this->model('ResourcesAttachment')->where($where)->count();
		if($count){
			$clienteteHandle = $this->model('ResourcesAttachment')->where($where)->orderby($orderby);
			$start = intval($start);
	...
	public function removeAttachmentId($attachmentId){
		...
		$attachmentData = $this->model('ResourcesAttachment')->where($where)->count();
		if($attachmentData){
	...
	public function insert($attachmentData){
		...
		return $this->model('ResourcesAttachment')->data($attachmentData)->add();
	}
}