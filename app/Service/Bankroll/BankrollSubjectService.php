 ...
 *
 */
class  BankrollSubjectService extends Service {
	...
	public function getSubjectList($where = array(),$start = 0,$perpage = 0,$orderby = 'identity desc'){
		...
		$count = $this->model('BankrollSubject')->where($where)->count();
		if($count){
			$subjectHandle = $this->model('BankrollSubject')->where($where)->orderby($orderby);
			if($perpage){
	...
	public function getSubjectIdByTitle($title){
		...
		$subjectData = $this->model('BankrollSubject')->field('identity')->where($where)->find();
		if(!$subjectData){		
	...
	public function getSubjectInfoById($subjectIds){
		...
		$subjectList = $this->model('BankrollSubject')->where($where)->select();
		if($subjectList){
	...
	public function removeSubjectId($subjectId){
		...
		$subjectData = $this->model('BankrollSubject')->where($where)->count();
		if($subjectData){
			
			$output = $this->model('BankrollSubject')->where($where)->delete();
		}
	...
	public function update($subjectNewData,$subjectId){
		...
		$subjectData = $this->model('BankrollSubject')->where($where)->find();
		if($subjectData){
			
			
			$subjectNewData['lastupdate'] = $this->getTime();
			$this->model('BankrollSubject')->data($subjectNewData)->where($where)->save();
	...
	public function insert($subjectData){
		...
		$subjectId = $this->model('BankrollSubject')->data($subjectData)->add();
