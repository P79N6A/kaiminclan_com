 ...
 *
 */
class  SecurityKeennessService extends Service {
	 ...
	 * @reutrn array;
	 */
	public function getKeennessList($where,$start,$perpage,$order = ''){
		
		$count = $this->model('SecurityKeenness')->where($where)->count();
		if($count){
			$listdata = $this->model('SecurityKeenness')->where($where)->orderby($order)->limit($start,$perpage,$count)->select();
		}
