 ...
 *
 *      $Id: Html.php 2016-07-28 10:21:25Z jianqimin $
 */
...
class filter_html
{
	...
	{
		//过滤链接
		$this->data = preg_replace('/<a([^>]*)>([^<]*)<\/a>/is', '\\2', $this->data);
		//过滤样式
		$this->data = preg_replace("/style=.+?['|\"]/i",'',$this->data);
		//过滤高
		$this->data = preg_replace("/width=.+?['|\"]/i",'',$this->data);
		$this->data = preg_replace("/heigh=.+?['|\"]/i",'',$this->data);
		
		//过滤JS代码
		$preg = '/<script[\s\S]*?<\/script>/i';
		$this->data = preg_replace($preg,'',$this->data);
		
		$output = $this->fixHtmlTag(array('html'=>$this->data));
		
		if(strpos($output,'<hr />') !== false)
		{
			$output = array_filter(explode('<hr />',$output));
			foreach($output as $key=>$content)
			{
				$output[$key] = htmlspecialchars($content);
			}
		}else{
	...
	* @since 2012-04-14
	*/
	private function fixHtmlTag($param = array()) 
	{
		//参数的默认值
		$html = '';
		$tagArray = array();
		...
		foreach ($contents as $tag) {
			...
			if(strpos($tag,'<iframe') !== false || strpos($tag,'</iframe>') !== false)
			{
				$result .= $tag;
				continue;
			}
